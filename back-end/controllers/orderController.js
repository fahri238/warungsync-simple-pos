...
const normalizeStatus = (status) => {
  ...
  return ORDER_STATUS[status.toLowerCase()] || null;
};
...
const getOrders = async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db.query(
      `
      SELECT
        o.id, o.id_pengguna, o.store_id, o.total_harga, o.komisi_sistem, o.status,
        o.tipe_pesanan, o.tipe_pengiriman, o.metode_pembayaran, o.bukti_transfer, o.tanggal_dibuat,
        u.nama AS nama_pelanggan, u.kontak AS no_hp_pelanggan,
        d.id AS delivery_id, d.id_kurir, d.alamat AS alamat_pengiriman, d.status AS status_pengiriman,
        oi.id AS item_id, oi.id_produk, oi.kuantitas, oi.harga,
        p.nama AS nama_produk, p.url_gambar
      FROM orders o
    ...
    for (const row of rows) {
      if (!grouped.has(row.id)) {
        grouped.set(row.id, {
          ...
          transferProof: row.bukti_transfer, // MENYERTAKAN BUKTI TF
...
const createOrder = async (req, res) => {
  ...
  // PERBAIKAN: Parsing JSON string dari FormData
  let items = req.body.items;
  if (typeof items === "string") {
    items = JSON.parse(items);
  }

  // Menangkap Foto Bukti Transfer dari Multer
  ...
  if (!resolvedStoreId || !currentUserId || !items || items.length === 0) {
    return res
      .status(400)
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (product.stok < qty)
        throw new Error(`Stok produk '${product.nama}' tidak mencukupi`);

      totalHarga += Number(product.harga) * qty;
    ...
    // INSERT BESERTA BUKTI TRANSFER
    const [orderResult] = await connection.query(
      `INSERT INTO orders 
      (id_pengguna, store_id, total_harga, komisi_sistem, status, tipe_pesanan, tipe_pengiriman, metode_pembayaran, bukti_transfer, tanggal_dibuat)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())`,
      [
        currentUserId,
        resolvedStoreId,
    ...
    if (tipe_pengiriman === "kurir") {
      ...
      await connection.query(
        "INSERT INTO deliveries (id_pesanan, id_kurir, alamat, destination_lat, destination_lng, status, tanggal_diperbarui) VALUES (?, ?, ?, ?, ?, 'diantar', NOW())",
        [
          insertedOrderId,
          assignedCourierId,
          alamat_pengiriman,
