...
const AdminDashboard = () => {
  ...
  // State untuk menampung data riil dari Backend nantinya
  ...
  useEffect(() => {
    // Keamanan ekstra: Pastikan yang masuk benar-benar Admin
    ...
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        // Tembak API statistik admin (Akan kita buatkan backend-nya nanti)
        ...
        // Fallback data kosong jika API belum siap, agar UI tetap terlihat
  ...
  // PENGECEKAN KEAMANAN (Akses Ditolak)
  if (!session || session.role !== "admin") {
    return (
        ...
        <Card className="w-full max-w-sm text-center shadow-2xl border-0 rounded-[2rem] overflow-hidden relative">
          ...
          <CardContent className="py-12">
            ...
            <p className="mb-8 text-muted-foreground text-sm px-4">
              Anda tidak memiliki otoritas tingkat Admin untuk mengakses Sistem Kontrol Pusat.
            </p>
  ...
  if (loading) {
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center text-muted-foreground">
        ...
        <p className="text-sm font-medium">Sinkronisasi data sistem global...</p>
      </div>
    );
  }

  const statCards = [
    { title: "Total Toko (Tenant)", value: data.totals.tenant, icon: Store, color: "text-blue-500", bg: "bg-blue-500/10" },
    { title: "Total Owner", value: data.totals.owner, icon: UserCheck, color: "text-indigo-500", bg: "bg-indigo-500/10" },
    { title: "Total Pelanggan", value: data.totals.pelanggan, icon: Users, color: "text-green-500", bg: "bg-green-500/10" },
    { title: "Total Kurir", value: data.totals.kurir, icon: Bike, color: "text-orange-500", bg: "bg-orange-500/10" },
    { title: "Total Produk", value: data.totals.produk, icon: Package, color: "text-purple-500", bg: "bg-purple-500/10" },
    { title: "Total Pesanan", value: data.totals.pesanan, icon: ShoppingCart, color: "text-primary", bg: "bg-primary/10" },
  ];

  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      {/* Banner Utama */}
      <div className="flex flex-col gap-2 rounded-2xl bg-primary/5 p-6 md:p-8 border border-primary/10 relative overflow-hidden">
        ...
        <p className="text-muted-foreground max-w-xl text-sm md:text-base leading-relaxed relative z-10">
          Selamat bertugas, <strong>{session.name}</strong>. Pantau kesehatan ekosistem WarungSync, mulai dari pertumbuhan pengguna, tenant, hingga lalu lintas transaksi hari ini.
        </p>
      </div>
      ...
      <div className="grid gap-4 grid-cols-2 md:grid-cols-3 xl:grid-cols-6">
        {statCards.map((c) => (
          <Card key={c.title} className="border-border/50 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Card className="border-border/50 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div className="space-y-1">
              <CardTitle className="text-lg font-bold">Lalu Lintas Transaksi</CardTitle>
              <p className="text-sm text-muted-foreground">Volume pesanan seluruh toko dalam 7 hari terakhir.</p>
            </div>
            ...
            <div className="h-[300px] w-full mt-4">
              {data.chartTransaksi.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={data.chartTransaksi} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    <defs>
                    ...
                    <Area type="monotone" dataKey="total" stroke="hsl(var(--primary))" strokeWidth={3} fillOpacity={1} fill="url(#colorPesanan)" />
                  </AreaChart>
                </ResponsiveContainer>
              )}
            </div>
        ...
        <Card className="border-border/50 shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <div className="space-y-1">
              <CardTitle className="text-lg font-bold">Pertumbuhan Pengguna Baru</CardTitle>
              <p className="text-sm text-muted-foreground">Registrasi user (Pelanggan, Kurir, Owner) per bulan.</p>
            </div>
          ...
          <CardContent>
            <div className="h-[300px] w-full mt-4">
              {data.chartPengguna.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data.chartPengguna} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="hsl(var(--border))" />
                    <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 12 }} dy={10} />
                    <YAxis axisLine={false} tickLine={false} tick={{ fill: "hsl(var(--muted-foreground))", fontSize: 12 }} />
                    <Tooltip contentStyle={{ borderRadius: "12px", border: "none", boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }} />
      ...
      <Card className="border-border/50 shadow-sm">
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Pantau pergerakan transaksi dan registrasi terbaru di seluruh sistem.
          </p>
        </CardHeader>
        <CardContent>
            ...
            <div className="space-y-4">
              {data.aktivitasTerbaru.map((log: any, idx) => (
                <div key={idx} className="flex items-start gap-4 p-3 rounded-lg hover:bg-muted/30 transition-colors">
                  <div className={`mt-0.5 h-2 w-2 rounded-full flex-shrink-0 ${log.type === 'register' ? 'bg-green-500' : 'bg-primary'}`} />
                  <div className="flex-1 space-y-1">
                    <p className="text-sm font-medium leading-none text-foreground">{log.pesan}</p>
