...
const OwnerDashboard = () => {
  ...
  // Ambil data produk dan pesanan berdasarkan hak akses Store ID Owner yang sedang login
  useEffect(() => {
    // PERUBAHAN: Cek session.role !== "owner"
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        console.error("Gagal memuat data dasbor owner:", error);
  ...
  // PENGECEKAN KEAMANAN (Akses Ditolak jika bukan Owner)
      ...
      <Card className="border-border/50 shadow-sm">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="outline" size="sm" asChild>
            {/* PERUBAHAN: Link ke /owner/orders */}
            <Link to="/owner/orders">Lihat Semua</Link>
          </Button>
        </CardHeader>
        <CardContent>
          {orders.length === 0 ? (
